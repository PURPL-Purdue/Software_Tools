...
def get_mdot_from_area(state, CdA, p_feed, p_c, rho, gamma=None):
    """
    ...
    if state == "gas":
        ...
        # --- Unchoked ---
            ...
            mdot = CdA * np.sqrt(2 * rho * p_feed * term)

        return mdot

    elif state == "liquid":
        # Bernoulli orifice equation
        mdot = CdA * np.sqrt(2 * rho * (p_feed - p_c))
        return mdot

    else:
        raise ValueError("Invalid state: choose 'gas' or 'liquid'")

# Function to parse a CSV at csv_dir/file_name and produce analysis parameters
# on the CSV. 
def data_regression(
    file_name: str,
    fuel_choice: str, #
